import { FormEventHandler, useState } from 'react';
import './LoginForm.css'
import * as EmailValidator from "email-validator";
import { FormField } from '../FormField';
import { Button } from '../../../ui/Button';
import { useMutation } from '@tanstack/react-query';
import { loginUser } from '../../../api/user';
import { queryClient } from '../../../api/queryClient';

export const LoginForm = () => {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");

  const [emailError, setEmailError] = useState<string | undefined>("");
  const [passwordError, setPasswordError] = useState<string | undefined>("");


  const handleSubmit: FormEventHandler<HTMLFormElement> = (e) => {
    e.preventDefault();

    if (!EmailValidator.validate(email)) {
      setEmailError("Введите корректный E-mail");
    } else if (password.length < 8) {
      setPasswordError("Пароль должен содержать не менее 8 символов");
    } else {
      loginMutation.mutate();
    }
  };

  const loginMutation = useMutation(
    {
      mutationFn: () => loginUser(email, password),
    },
    queryClient
  );

  return (
    <form className="login-form" onSubmit={handleSubmit}>
      <FormField>
        <input
          className='login-form__inp'
          type="text"
          name="email"
          placeholder='Электронная почта'
          onChange={(e) => {
            setEmail(e.target.value);
            setEmailError(undefined);
          }}
          value={email}
          required
        />
        <svg className='login__inp-mail' width="24" height="24" viewBox="0 0 24 24" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path d="M21 3C21.5523 3 22 3.44772 22 4V20.0066C22 20.5552 21.5447 21 21.0082 21H2.9918C2.44405 21 2 20.5551 2 20.0066V19H20V7.3L12 14.5L2 5.5V4C2 3.44772 2.44772 3 3 3H21ZM8 15V17H0V15H8ZM5 10V12H0V10H5ZM19.5659 5H4.43414L12 11.8093L19.5659 5Z" />
        </svg>
      </FormField>
      {emailError && (
        <span style={{ color: "red", fontSize: "14px" }}>{emailError}</span>
      )}
      <FormField>
        <input
          className='login-form__inp'
          type="password"
          name="password"
          placeholder='Пароль'
          onChange={(e) => {
            setPassword(e.target.value);
            setPasswordError(undefined);
          }}
          value={password}
          required
        />
        <svg className='login__inp-key' width="24" height="24" viewBox="0 0 24 24" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path d="M12.917 13C12.441 15.8377 9.973 18 7 18C3.68629 18 1 15.3137 1 12C1 8.68629 3.68629 6 7 6C9.973 6 12.441 8.16229 12.917 11H23V13H21V17H19V13H17V17H15V13H12.917ZM7 16C9.20914 16 11 14.2091 11 12C11 9.79086 9.20914 8 7 8C4.79086 8 3 9.79086 3 12C3 14.2091 4.79086 16 7 16Z" />
        </svg>
      </FormField>
      {passwordError && (
        <span style={{ color: "red", fontSize: "14px" }}>{passwordError}</span>
      )}
      {loginMutation.error && <span>{loginMutation.error.message}</span>}
      <Button className='login-form__btn btn btn-reset' type="submit" >
        Войти
      </Button>
    </form>
  );
}
